...
import { borrowPageDashboardData, lendPageDashboardData } from '../../mocks/dashboardsData';
import { lendStablecoinsData } from '../../mocks/tableData';
import Table from '../Table/Table';
import { borrowLiquidityItem } from '../../mocks/borrowLiquidity';
...
export default function Transfer({ page }: TransferProps) {
  ...
  const [transactionData, setDataTransaction] = useState<iTransactionData | null>(null);
  ...
  const methodType = page == PageType.Lend ? MethodType.addLiquidity : MethodType.initBorrowerLoan;
  ...
  useEffect(() => {
    if (evmWallet && evmWallet.networkId) {
      ...
      setFormData({
        ...
        crypto: getStableCoin(currentNetwork),
  ...
  return (
    <div className={style.page}>
      ...
      <Dashboard page={page} lendItem={lendPageDashboardData} borrowItem={borrowPageDashboardData} stableCoin={stableCoin} />
      <Table page={page}
        itemLend={lendStablecoinsData} itemBorrow={borrowLiquidityItem} stableCoin={stableCoin} formData={formData} setAmount={setAmount}
